...
export function AccountList({ accounts }: { accounts: DashboardAccount[] }) {
  if (accounts.length === 0) {
    return <div className="rounded-md border border-dashed border-stone-300 p-6 text-center text-sm text-stone-500">No accounts yet.</div>;
  }
    ...
    <div className="min-w-0 divide-y divide-stone-100">
      {accounts.map((account) => (
        <Link
          ...
          className="group flex min-h-14 min-w-0 items-center gap-3 py-4 first:pt-0 last:pb-0 sm:gap-4"
        >
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-red-50 text-xs font-bold text-(--brand-red)">
            {account.type.slice(0, 2)}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-stone-900">
              {account.type.charAt(0) + account.type.slice(1).toLowerCase()} account
            </p>
            <p className="mt-1 text-xs text-stone-500">•••• {account.accountNumber.slice(-4)}</p>
          </div>
          <div className="hidden text-right sm:block">
            <p className="text-xs text-stone-500">Status</p>
            <p className="mt-1 flex items-center justify-end gap-1 text-xs font-semibold text-emerald-700"><CheckCircle2 size={13} /> Active</p>
          </div>
          <p className="shrink-0 whitespace-nowrap text-xs font-bold text-stone-900 sm:text-sm">
            {formatMoney(account.balance)}
          </p>
          <ChevronRight size={18} className="shrink-0 text-stone-300 transition group-hover:translate-x-0.5 group-hover:text-(--brand-red)" />
        </Link>
      ))}
    </div>
  );
